...
import threading
import time
from db_functions import get_P2_detailed_classification
import curses
import time
...
class ClassificationMonitor:
    def __init__(self, func, stdscr, refresh_rate=10, rows=50, old_refresh_rate=60):
        ...
        self.column_modes = {
            "NORMAL": list(range(0, 8)) + [56],
            #"ONLY_TIMES": range(8),
            "P_OBJS": list(range(0,6)) + list(range(32, 42)),
            "H_OBJS": list(range(0,6)) + list(range(42, 52)),
            "S_OBJS": list(range(0,6)) + list(range(52, 56)), 
            "P_POINTS": list(range(0,6)) + list(range(8, 18)),
            "H_POINTS": list(range(0,6)) + list(range(18, 28)),
            "S_POINTS": list(range(0,6)) + list(range(28, 32)),
            #"SCORES": list(range(0, 6)) + list(range(8, 16)),
            #"REAL": list(range(0, 6)) + list(range(16, 24)),
            #"BY_LEVELS": list(range(0, 6)) + list(range(12, 16)) + list(range(20, 24)),
            #"ONLY_LEVEL_SCORES": list(range(0, 6)) + list(range(8, 12)),
            #"ONLY_SIZE_SCORES": list(range(0, 6)) + list(range(12, 16)),
            #"ONLY_LEVEL_REAL": list(range(0, 6)) + list(range(16, 20)),
            #"ONLY_SIZE_REAL": list(range(0, 6)) + list(range(20, 24)),
            "MINIMAL": range(0, 6),
        }
                ...
                if i == 0:
                    ...
                    if self.last_df is not None:
                        ...
                        if len(old) > 0:
                            ...
                            elif int(curr.name) == int(old.name) and curr['leaderboard_score'] > old['leaderboard_score']:
                                self.screen.addstr(i, 0, line, curses.A_BOLD | curses.color_pair(2))
...
def main(stdscr):
    ...
    try:
        ...
        monitor = ClassificationMonitor(get_P2_detailed_classification, stdscr, refresh_rate=10, rows=50, old_refresh_rate=300)
        monitor.stop = True
        monitor.thread.join()
