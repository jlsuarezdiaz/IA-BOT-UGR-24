...
async def main():
    ...
    if error is None:
        ...
        if len(wrong_entregas) > 0:
            error = "⚠️ Hay entregas que no tienen los archivos correctos: " + ", ".join(wrong_entregas)
            error += "\nNecesaria revisión manual. Por favor, corrige o elimina las entregas con errores y vuelve a subir el zip."
    ...
    if error is None:
        msg = "✅ Ficheros descomprimidos correctamente"
        async with bot:
